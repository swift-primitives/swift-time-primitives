...
struct JulianTests {
    ...
    func unixEpochConstant() {
        ...
        #expect(jd.rawValue == 2_440_587.5)
    }

    @Test
    func j2000Constant() {
        let jd = Time.Julian.Day.j2000
        #expect(jd.rawValue == 2_451_545.0)
    }

    @Test
    func modifiedOffsetConstant() {
        let offset = Time.Julian.Offset.modified
        #expect(offset.rawValue == 2_400_000.5)
    }
    ...
    func timeToJulianDayJ2000() throws {
        ...
        #expect(abs(jd.rawValue - 2_451_545.0) < 0.0001)
    }
    ...
    func timeToJulianDayUnixEpoch() throws {
        ...
        let jd = Time.Julian.Day(time)
        #expect(abs(jd.rawValue - 2_440_587.5) < 0.0001)
    }
    ...
    func timeToJulianDayMidnight() throws {
        ...
        let jd = Time.Julian.Day(time)
        let fractionalPart = jd.rawValue - Double(Int(jd.rawValue))
    ...
    func dayMinusDayEqualsOffset() {
        ...
        let jd2 = Time.Julian.Day(__unchecked: (), 2_451_546.0)
        let offset = jd2 - jd1
        #expect(abs(offset.rawValue - 1.0) < 0.0001)
    }

    @Test
    func dayPlusOffsetEqualsDay() {
        let jd = Time.Julian.Day.j2000
        let offset = Time.Julian.Offset(__unchecked: (), 10.0)
        let result = jd + offset
        #expect(abs(result.rawValue - 2_451_555.0) < 0.0001)
    }
    ...
    func dayMinusOffsetEqualsDay() {
        ...
        let offset = Time.Julian.Offset(__unchecked: (), 10.0)
        let result = jd - offset
        #expect(abs(result.rawValue - 2_451_535.0) < 0.0001)
    }
    ...
    func modifiedJulianDayFromOffset() {
        let jd = Time.Julian.Day.j2000
        let mjd = jd - .modified
        #expect(abs(mjd.rawValue - 51544.5) < 0.0001)
    }
